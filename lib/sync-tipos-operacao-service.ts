...
async function buscarTiposOperacaoSankhya(idSistema: number, bearerToken: string): Promise<TipoOperacaoSankhya[]> {
  ...
  if (!entities || !entities.entity) {
    console.log(`⚠️ [Sync] Nenhum tipo de operação encontrado para ID_SISTEMA: ${idSistema}`);
    return [];
  }
  ...
  console.log(`✅ [Sync] ${tiposOperacao.length} tipos de operação recuperados do Sankhya`);
