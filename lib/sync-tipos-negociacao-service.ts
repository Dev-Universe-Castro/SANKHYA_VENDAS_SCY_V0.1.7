...
async function buscarTiposNegociacaoSankhya(idSistema: number, bearerToken: string): Promise<TipoNegociacaoSankhya[]> {
  ...
  if (!entities || !entities.entity) {
    console.log(`⚠️ [Sync] Nenhum tipo de negociação encontrado para ID_SISTEMA: ${idSistema}`);
    return [];
  }
  ...
  console.log(`✅ [Sync] ${tiposNegociacao.length} tipos de negociação recuperados do Sankhya`);
